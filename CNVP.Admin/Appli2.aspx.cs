            ...
            if (!IsPostBack)
            {
                ...
                if (Cookie != null)
                {
                    try
                    {
                        ...
                        DataTable dt = DbHelper.ExecutePage("A.ID,A.AppName,B.AppContent,B.AppReply, B.PostTime,B.IsAudit,B.AppContent,B.AppThings", StrWhere, "B.ID", "Order By B.PostTime Desc", Convert.ToInt32(PageNo), PageSize, out RecordCount, out PageCount);
