...
namespace CNVP.Admin
{
    public partial class Ajax : System.Web.UI.Page
    {
        ...
        public string SendSms2(string UserPhone)
        {
            ...
            string key = "cnvp";
            long unknow = Convert.ToInt64(DateTime.Now.ToString("yyyyMMddHHmmssfff"));
            string SmsContent = "退订回复TD【鹿城档案地方志网】";
            return Ajax.CreateService().sendSMS(SerialNum, key, "", SmsPhone, SmsContent, "", "", 5, unknow).ToString();
        }
        ...
        {
            string ID = Request.Params["value"];
            if (string.IsNullOrEmpty(ID) || !Public.IsNumber(ID))
            {
                ID = "0";
            }
            using (DataTable dt = DbHelper.ExecuteTable("select * from HX_Type where ID=" + ID))
            {
                if (dt != null && dt.Rows.Count > 0)
                {
                    Response.Write("" + dt.Rows[0]["TypeContent"].ToString() + "");
                    Response.End();
                }
                else
                {
