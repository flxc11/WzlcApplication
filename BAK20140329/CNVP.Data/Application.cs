    ...
    public class Application
    {
        public void AddApplication(Model.Application Model)
        {
            string StrSql = "insert into " + DbConfig.Prefix +
                "Application (AppType, AppPic, AppResult, PostTime, IsAudit, AppContent, AppUserID) values (@AppType, @AppPic, @AppResult, @PostTime, @IsAudit, @AppContent, @AppUserID)";
        ...
        {
            string StrSql = "update " + DbConfig.Prefix + "Application set AppReply=@AppReply where ID=@ID";
            IDataParameter[] Param = new IDataParameter[] {
