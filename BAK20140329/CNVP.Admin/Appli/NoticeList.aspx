<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NoticeList.aspx.cs" Inherits="CNVP.Admin.Appli.NoticeList" %>
...
<script type="text/javascript">
    ...
    var grid = $("#maingrid").ligerGrid({
        columns: [
                ...
                { display: '事项名称', name: 'NoticeTitle', align: 'center', width: 150, minWidth: 60 },
                { display: '添加时间', name: 'PostTime', align: 'center', width: 500, minWidth: 500 }
        ], dataAction: 'server', pageSize: 20, toolbar: toolbarOptions, sortName: 'ID',
        width: '100%', height: '100%', heightDiff: -5, url: 'NoticeList.aspx?Action=List', checkbox: true, usePager: true, whenRClickToSelect: true, fixedCellHeight: true, rowHeight: 25,
    ...
    function itemclick(item) {
        ...
        switch (item.text) {
            case "增加":
                top.f_addTab('TypeAdd', '增加事项', 'Appli/NoticeAdd.aspx');
                break;
            case "移除":
                if (selected != null) {
    ...
    function f_delete() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "NoticeList.aspx?Action=Del&Time=" + (new Date().getTime()),
            data: params,
            error: function () {
    ...
    function f_onDblClickRow(data, rowid, rowdata) {
        top.f_addTab(null, '修改内容', 'Appli/NoticeEdit.aspx?ID=' + data.ID);
    }
  </script>
</body>
